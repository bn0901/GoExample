...
func worker(workerName string, jobs <-chan int, results chan<- int) {
	for j := range jobs {  // jobs是通道，是并发安全的。所以
		fmt.Println("worker", workerName, "started  job", j)
		time.Sleep(time.Second)
		fmt.Println("worker", workerName, "finished job", j)
		results <- j * 2
	}
}
...
func main() {
 	// 现在有3个工人，总共要做5个任务
	workers := map[int]string{1:"小1明", 2:"小2李", 3:"小3王"}
	...
	for w := 1; w <= 3; w++ {
		go worker(workers[w], jobs, results)
	}
